...
.controller('DashCtrl', function ($scope) {
    ...
    var signalling_server_hostname = "192.168.1.11";//location.hostname || "<%server_ip%>";
    var signalling_server_address = "192.168.1.11:8080";//signalling_server_hostname + ':' + (location.port || 80);
    ...
    $scope.sendCaptureCmd = function(cmd){    
        ...
        var address = "http://192.168.1.11/";
        console.log("xhr sent...");

        if (cmd === "4") {
            xmlhttp.open("POST", address + "server/restart", true);
        }
        else {
